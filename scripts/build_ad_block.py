...
LEGACY_SOURCE_FILE = ROOT / "data" / "sources" / "ad_legacy_rule_seed_urls.txt"
...
def load_legacy_sources() -> list[str]:
    if not LEGACY_SOURCE_FILE.exists():
        return []
    sources: list[str] = []
    for raw in LEGACY_SOURCE_FILE.read_text(encoding="utf-8").splitlines():
        line = raw.strip()
...
LEGACY_SOURCES = load_legacy_sources()
...
def run_legacy_standalone(output: Path = LEGACY_OUTPUT) -> int:
    ...
    if not sources:
        print(f"No legacy sources found in {LEGACY_SOURCE_FILE}", file=sys.stderr)
        return 1
...
def main() -> int:
    ...
    print(f"Legacy sources now live in {LEGACY_SOURCE_FILE.relative_to(ROOT)}")
